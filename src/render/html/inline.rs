...
pub(crate) fn render_inlines(inlines: &[Inline], rc: &Ctx) -> String {
    ...
    for inline in inlines {
        match inline {
            ...
            Inline::Image { alt, source } => render_image(alt, source, rc, &mut out),
            Inline::Anchor(id) => {
...
fn render_image(alt: &str, source: &ImageSource, rc: &Ctx, out: &mut String) {
    match source {
        ...
        // An exported illustration renders as an `<img>` with a file source;
        // anything else renders as its alt text only.
